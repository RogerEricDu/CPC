// Adapted for CPC from SequenceTubeMap. See LICENSE.txt in this directory.
...
function defaultTrackColors(trackType) {
  const key = trackType === "node" ? "haplotype" : trackType;
  if (!DEFAULT_TRACK_COLORS[key]) {
    throw new Error("Invalid track type: " + trackType);
  }
  return { ...DEFAULT_TRACK_COLORS[key] };
}

const DEBUG = false;

const QTL_TYPES = new Map([
  ["caqtl", "caQTL"],
  ["eqtl", "eQTL"],
]);

function normalizeQTLType(value) {
  if (typeof value !== "string") return null;
  return QTL_TYPES.get(value.toLowerCase()) || null;
}

// Read QTL metadata from structured GAF tags, while retaining compatibility
// with the pipe-delimited names produced by scripts/qtl_to_gaf.py.
export function extractQTLMetadata(read) {
  const tags = read.gaf_tags || {};
  const nameParts = typeof read.name === "string" ? read.name.split("|") : [];
  const type = normalizeQTLType(
    read.annotation_type || tags.qt || nameParts[0]
  );
  if (!type) return null;

  const namedValues = {};
  for (const part of nameParts.slice(3)) {
    const separator = part.indexOf("=");
    if (separator !== -1) {
      namedValues[part.slice(0, separator)] = part.slice(separator + 1);
    }
  }

  return {
    type,
    phenotype: tags.ph || nameParts[1] || null,
    variant: tags.gv || nameParts[2] || null,
    pvalue: tags.pv || namedValues.p || null,
    slope: tags.sl || namedValues.beta || null,
    slopeSE: tags.se || null,
    alleleFrequency: tags.af || null,
    startDistance: tags.sd || null,
    endDistance: tags.ed || null,
    minorAlleleSamples: tags.ms || null,
    minorAlleleCount: tags.mc || null,
  };
}

export function isQTLTrack(track) {
  return Boolean(track && (track.qtl || extractQTLMetadata(track)));
}
...
let resizeObserver;
let wheelParent;
const preventVisualizationScroll = (event) => event.preventDefault();
let inputNodes = [];
let inputTracks = [];
let inputReads = [];
let inputRegion = [];
...
function createTubeMap() {
  if (DEBUG) console.log("Recreating tube map in", svgID);
  trackRectangles = [];
  trackCurves = [];
  trackCorners = [];
  ...
  // QTLs keep STM's native filled-ribbon geometry. A separate vector layer
  // draws only the exact upper and lower ribbon boundaries, after the fills
  // but before nodes. Deliberately omitting vertical joins prevents seams at
  // node boundaries and partial-read endpoints in exported SVGs.
  const qtlBoundaryGroup = trackGroup
    .append("g")
    .attr("class", "qtl-track-boundaries");
  drawQTLTrackBoundaries(trackRectangles, trackCurves, qtlBoundaryGroup);
...
function generateReadOnlyNodeAttributes() {
  ...
  nodes.forEach((node, i) => {
    if (node.hasOwnProperty("order") && !node.hasOwnProperty("y")) {
      ...
      node.contentHeight = getNodeSequencePadding();
      nodesPerOrder[node.order].push(i);
    }
  });
}

function getNodeSequencePadding() {
  return config.nodeWidthOption === "normal" ? 16 : 0;
}
...
const READ_WIDTH = 7;
const QTL_WIDTH = 12;
const QTL_LANE_GAP = 4;
const READ_TRACK_MARGIN = 5;
...
function assignReadsToNodes() {
  ...
  reads.forEach((read, idx) => {
    read.width = isQTLTrack(read) ? QTL_WIDTH : READ_WIDTH;
...
function placeReads() {
  ...
  // Annotation tracks belong below sequencing reads. This also keeps QTL
  // paths at the bottom edge of nodes, visually separated from haplotypes.
  allSources.sort((a, b) => {
    const aIsQTL = readsBySource[a].some((index) => isQTLTrack(reads[index]));
    const bIsQTL = readsBySource[b].some((index) => isQTLTrack(reads[index]));
    if (aIsQTL !== bIsQTL) return aIsQTL ? 1 : -1;
    return Number(a) - Number(b);
  });

  if (DEBUG) console.log("All sources: ", allSources);

  // Space out read tracks if multiple exist
  let topMargin = allSources.length > 1 ? READ_TRACK_MARGIN : 0;
  ...
  elementsWithoutNode.forEach((element) => {
    ...
    bottomY[segment.order] += reads[element.readIndex].width + 2;
  });
...
function placeReadSet(readIDs, node, topMargin) {
  ...
  const containsQTL = readIDs.some((readID) => isQTLTrack(reads[readID]));
  const laneHeight =
    Math.max(
      READ_WIDTH,
      ...readIDs.map((readID) => reads[readID].width || READ_WIDTH)
    ) + (containsQTL ? QTL_LANE_GAP : 2);
    ...
    if (
      ...
      incomingReads.forEach((incReadElementIndices) => {
        ...
        if (incReadPathElement.y >= currentY) {
          incReadPathElement.y += laneHeight;
          setOccupiedUntil(
            occupiedUntil,
            incRead,
...
function alignSVG() {
  ...
  if (wheelParent) {
    wheelParent.removeEventListener("wheel", preventVisualizationScroll);
  }
  wheelParent = parentElement;
  wheelParent.addEventListener("wheel", preventVisualizationScroll, {
    passive: false,
  });
...
export function resetZoom() {
  if (!svgID || !zoom) return;
  const svgElement = document.getElementById(svgID.substring(1));
  if (!svgElement || !svgElement.parentNode) return;
  const parentElement = svgElement.parentNode;
  const xOffset =
    maxXCoordinate + 10 < parentElement.clientWidth
      ? (parentElement.clientWidth - maxXCoordinate - 10) / 2
      : 0;
  d3.select(svgID)
    .transition()
    .duration(250)
...
export function destroy() {
  ...
  if (wheelParent) {
    wheelParent.removeEventListener("wheel", preventVisualizationScroll);
    wheelParent = undefined;
  }
  if (svgID) d3.select(svgID).on(".zoom", null).selectAll("*").remove();
  svg = undefined;
  svgID = undefined;
  zoom = undefined;
  inputNodes = [];
...
function generateSingleLaneAssignment(assignment, order) {
  ...
  assignment.forEach((node) => {
    if (node.node !== null) {
      ...
      nodes[node.node].contentHeight = getNodeSequencePadding();
      currentY += getNodeSequencePadding();
...
function generateTrackColor(track, highlight, qtlSignificanceExtent) {
  ...
  if (
    qtlSignificanceExtent &&
    track.hasOwnProperty("type") &&
    track.type === "read" &&
    isQTLTrack(track)
  ) {
    const qtl = track.qtl || extractQTLMetadata(track);
    const colorStrength = qtlColorStrengthFromPValue(
      qtl?.pvalue,
      qtlSignificanceExtent
    );
    trackColor = d3.interpolateRgb("#ffffff", trackColor)(colorStrength);
  }
...
function generateTrackAlpha(track, highlight) {
  ...
  if (track.hasOwnProperty("type") && track.type === "read") {
    if (
      !isQTLTrack(track) &&
      config.colorSchemes[sourceID].alphaReadsByMappingQuality
    ) {
...
function qtlSignificanceFromPValue(pvalue) {
  if (pvalue === null || pvalue === undefined || pvalue === "") return null;

  const numericPValue = Number(String(pvalue).trim().replace(/^<\s*/, ""));
  if (
    Number.isNaN(numericPValue) ||
    numericPValue <= 0 ||
    numericPValue > 1
  ) {
    return null;
  ...
  return -Math.log10(numericPValue);
}

function getQTLSignificanceExtent(trackList) {
  const values = trackList
    .filter((track) => track.type === "read" && isQTLTrack(track))
    .map((track) => {
      const qtl = track.qtl || extractQTLMetadata(track);
      return qtlSignificanceFromPValue(qtl?.pvalue);
    })
...
// Scale each visible QTL against the current QTL significance range. The
// result controls how much of the configured QTL color is mixed into white.
// Keep even the least significant visible QTL at 40% color strength so it
// remains distinct from the pastel haplotype context. Using an opaque mixed
// color also prevents overlapping SVG segments from creating darker seams.
export function qtlColorStrengthFromPValue(pvalue, significanceExtent) {
  const significance = qtlSignificanceFromPValue(pvalue);
  if (significance === null || !significanceExtent) return 1;

  const [minSignificance, maxSignificance] = significanceExtent;
  if (maxSignificance <= minSignificance) return 1;
...
function generateSVGShapesFromPath() {
  ...
  let dummy;
  let reversalFlag;
  const qtlSignificanceExtent = getQTLSignificanceExtent(tracks);
...
function getNodeByName(nodeName) {
  ...
  // We just do a scan.
  // TODO: index!
  if (DEBUG) console.log("All nodes:", nodes); //
  for (let i = 1; i < nodes.length; i++) {
    // changes i to start from 1 instead of 0
    if (nodes[i].name === nodeName) {
      if (DEBUG) console.log("Found node with name", nodeName, "at index ", i);
      return nodes[i];
    }
  }
}
...
function nodeSingleClick() {
  ...
  if (DEBUG) console.log("Node ", nodeName, " is ", currentNode);
  ...
  if (DEBUG) console.log("node show info callback", config.showInfoCallback);
  ...
  for (let readVisit of node.incomingReads) {
    ...
    for (let i = 0; i < currRead.sequenceNew.length; i += 1) {
      currRead.sequenceNew[i].mismatches.forEach((mm) => {
        if (mm.type === "deletion") {
          if (DEBUG) console.log("this read has a deletion", currRead);
...
const QTL_BOUNDARY_WIDTH = 1.25;

function isQTLShape(shape) {
  return (
    shape &&
    shape.type === "read" &&
    isQTLTrack({ name: shape.name })
  );
}

// Build one compound open boundary path per QTL from the same upper and lower
// geometry as its filled ribbon. The boundary intentionally has no vertical
// sides, so SVG segments can meet at nodes without producing dark join lines.
export function buildQTLBoundaryOutlines(rectangles, curves) {
  const boundariesByTrack = new Map();

  function addPathPart(shape, path) {
    const trackID = String(shape.id);
    if (!boundariesByTrack.has(trackID)) {
      boundariesByTrack.set(trackID, {
        id: shape.id,
        name: shape.name,
        pathParts: [],
      });
    }
    boundariesByTrack.get(trackID).pathParts.push(path);
  }

  rectangles.filter(isQTLShape).forEach((rectangle) => {
    addPathPart(
      rectangle,
      `M ${rectangle.xStart} ${rectangle.yStart} H ${rectangle.xEnd + 1}`
    );
    addPathPart(
      rectangle,
      `M ${rectangle.xStart} ${rectangle.yEnd + 1} H ${rectangle.xEnd + 1}`
    );
  });

  curves.filter(isQTLShape).forEach((curve) => {
    if (curve.topBoundaryPath) addPathPart(curve, curve.topBoundaryPath);
    if (curve.bottomBoundaryPath) addPathPart(curve, curve.bottomBoundaryPath);
  });

  return Array.from(boundariesByTrack.values()).map((boundary) => ({
    id: boundary.id,
    name: boundary.name,
    path: boundary.pathParts.join(" "),
  }));
}

function drawQTLTrackBoundaries(rectangles, curves, boundaryGroup) {
  const boundaries = buildQTLBoundaryOutlines(rectangles, curves);

  boundaryGroup
    .selectAll("qtlBoundaryOutline")
    .data(boundaries)
    .enter()
    .append("path")
    .attr("d", (d) => d.path)
    .attr("class", "qtl-track-boundary")
    .attr("fill", "none")
    .attr("stroke", "#202020")
    .attr("stroke-width", QTL_BOUNDARY_WIDTH)
...
function drawTrackRectangles(rectangles, type, groupTrack) {
  ...
  groupTrack
    ...
    .attr(
      ...
      (d) =>
        `track${d.id}${
          type === "read" && isQTLTrack({ name: d.name }) ? " qtl-track" : ""
        }`
    )
  ...
  Object.values(groupedCurves).forEach((curveGroup) => {
    ...
    curveGroup.forEach((curve) => {
      ...
      if (curve.yStart < curve.yEnd) {
        ...
        adjustValue += adjustIncrement;
        xNextAdjusted =
          curve.xStart + (curve.xEnd - curve.xStart) * adjustValue;
      }
      ...
      d += " Z";
      curve.path = d;
      curve.topBoundaryPath =
        `M ${curve.xStart} ${curve.yStart}` +
        ` C ${xAdjusted} ${curve.yStart}` +
        ` ${xAdjusted} ${curve.yEnd}` +
        ` ${curve.xEnd} ${curve.yEnd}`;
      curve.bottomBoundaryPath =
        `M ${curve.xStart} ${curve.yStart + curve.width}` +
        ` C ${xNextAdjusted} ${curve.yStart + curve.width}` +
        ` ${xNextAdjusted} ${curve.yEnd + curve.width}` +
        ` ${curve.xEnd} ${curve.yEnd + curve.width}`;
    });
  });
...
function getTrackByID(trackID) {
  ...
  for (let i = 0; i < tracks.length; i++) {
    if (tracks[i].id === trackID) {
      if (DEBUG) console.log("Found track with ID ", trackID, " at index ", i);
      return tracks[i];
    }
  }
}
...
export function highlightTrackByID(trackID) {
  if (!svgID) return;
  d3.select(svgID).selectAll(`.track${trackID}`).style("fill", "url(#patternA)");
}

export function clearTrackHighlightByID(trackID) {
  if (!svgID) return;
  d3.select(svgID).selectAll(`.track${trackID}`).each(function clearTrackHighlight() {
...
function addAttribute(attributes, label, value) {
  if (value !== null && value !== undefined && value !== "") {
    attributes.push([label, value]);
  }
}

function splitVariantID(variant) {
  ...
  return {
    ...
    position: parts[1],
    reference: parts[2],
    effect: parts.slice(3).join("_"),
  };
}

export function getTrackInfoAttributes(track) {
  const qtl = track.qtl || extractQTLMetadata(track);
  if (qtl) {
    const variant = splitVariantID(qtl.variant);
    const attributes = [];
    addAttribute(attributes, "QTL Type", qtl.type);
    addAttribute(attributes, "Phenotype", qtl.phenotype);
    addAttribute(attributes, "Variant ID", qtl.variant);
    addAttribute(attributes, "Chromosome", variant.chromosome);
    addAttribute(attributes, "Position", variant.position);
    addAttribute(attributes, "Reference Allele", variant.reference);
    addAttribute(attributes, "Effect Allele", variant.effect);
    addAttribute(attributes, "P-value", qtl.pvalue);
    addAttribute(attributes, "Effect (Beta)", qtl.slope);
    addAttribute(attributes, "Slope SE", qtl.slopeSE);
    addAttribute(attributes, "Allele Frequency", qtl.alleleFrequency);
    addAttribute(attributes, "Minor Allele Samples", qtl.minorAlleleSamples);
    addAttribute(attributes, "Minor Allele Count", qtl.minorAlleleCount);
    addAttribute(attributes, "Start Distance", qtl.startDistance);
    addAttribute(attributes, "End Distance", qtl.endDistance);
    addAttribute(attributes, "Graph Path", getPathInfo(track));
    addAttribute(attributes, "GAF Record Name", track.name);
    return attributes;
  }

  const attributes = [["Name", track.name]];
  if (track.type === "read") {
    attributes.push(["Sample Name", track.sample_name]);
    attributes.push([
      "Primary Alignment?",
      track.is_secondary ? "Secondary" : "Primary",
    ]);
    attributes.push(["Read Group", track.read_group]);
    attributes.push(["Score", track.score]);
    attributes.push(["CIGAR string", track.cigar_string]);
    attributes.push(["Mapping Quality", track.mapping_quality]);
    attributes.push(["Path Info", getPathInfo(track)]);
  }
  return attributes;
}
...
function trackSingleClick() {
  ...
  if (DEBUG) console.log("Track ", trackID, " is ", current_track);
  if (current_track === undefined) {
    console.error("Missing track: ", trackID);
    return;
  }
  const track_attributes = getTrackInfoAttributes(current_track);
  if (DEBUG) console.log("Single Click");
  if (DEBUG) console.log("read path");
  config.showInfoCallback(
    track_attributes,
    isQTLTrack(current_track) ? "QTL Information" : "Object Information"
  );
}
...
function generateNodeWidth() {
  ...
  switch (config.nodeWidthOption) {
    ...
    case "fixed":
      ...
      nodes.forEach((node) => {
        if (DEBUG) console.log("node.sequenceLength:", node.sequenceLength);
        node.width = 10;
...
export function vgExtractTracks(vg, pathSourceTrackId, haplotypeSourceTrackID) {
  ...
  vg.path.forEach((path, index) => {
    ...
    const firstPathName = path.name ? path.name.split(",")[0].trim() : null;
    const isNamedHaplotype =
      firstPathName &&
      (firstPathName.startsWith("thread_") ||
        (!firstPathName.startsWith("GRCh38#") &&
          !firstPathName.startsWith("CHM13#") &&
          firstPathName.includes("#")));
    if (isNamedHaplotype) {
      // This is a haplotype
...
export function vgExtractReads(
  ...
  for (let i = 0; i < myReads.length; i += 1) {
    ...
    if (sequence.length === 0) {
      ...
      if (read.hasOwnProperty("gaf_tags")) track.gaf_tags = read.gaf_tags;
      if (read.hasOwnProperty("annotation_type")) {
        track.annotation_type = read.annotation_type;
      }
      const qtl = extractQTLMetadata(read);
      if (qtl) {
        track.qtl = qtl;
        track.annotation_type = qtl.type;
      }
